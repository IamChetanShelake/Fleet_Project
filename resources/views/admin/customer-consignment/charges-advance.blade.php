...
<div class="dashboard-wrapper">
                ...
                <div class="trip-details-panel">
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-stats-row">
                            <div class="trip-stat-item">
                                <div class="trip-stat-value" id="trip-stat-distance">{{ $consignment->total_distance ? number_format($consignment->total_distance, 2) : '--' }}</div>
                                <div class="trip-stat-label">Distance (km)</div>
                            </div>
                            <div class="trip-stat-item">
                                <div class="trip-stat-value" id="trip-stat-time">{{ $consignment->total_travel_time ?? '--' }}</div>
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-location-marker">
                            ...
                            <div class="trip-location-text">
                                ...
                                <div class="trip-location-address" id="trip-pickup-address">{{ $consignment->pickup_location ?? 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="trip-location-marker">
                            ...
                            <div class="trip-location-text">
                                ...
                                <div class="trip-location-address" id="trip-delivery-address">{{ $consignment->delivery_location ?? 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-detail-row">
                            ...
                            <span class="trip-detail-value" id="trip-pickup-time">{{ $consignment->pickup_datetime ? $consignment->pickup_datetime->format('d M Y, h:i A') : 'N/A' }}</span>
                        </div>
                        <div class="trip-detail-row">
                            <span class="trip-detail-label">Expected Delivery</span>
                            <span class="trip-detail-value" id="trip-delivery-date">{{ $consignment->delivery_date ? $consignment->delivery_date->format('d M Y') : 'N/A' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-driver-info">
                            ...
                            $driverPhoto = '';
                            $driverId = null;
                            if(isset($consignment) && $consignment->assigned_driver_id) {
                                $driver = \App\Models\Driver::find($consignment->assigned_driver_id);
                                $driverId = $consignment->assigned_driver_id;
                            ...
                            <div class="trip-driver-details">
                                <div class="trip-driver-name">{{ $consignment->assigned_driver ?? 'N/A' }}</div>
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-vehicle-info">
                            ...
                            $vehiclePhoto = '';
                            $vehicleId = null;
                            if(isset($consignment) && $consignment->assigned_vehicle_no) {
                                $vehicle = \App\Models\Vehicle::where('vehicle_number', $consignment->assigned_vehicle_no)->first();
                                if($vehicle) {
                            ...
                            <div class="trip-vehicle-details">
                                <div class="trip-vehicle-number">{{ $consignment->assigned_vehicle_no ?? 'N/A' }}</div>
                                <div class="trip-vehicle-type">{{ $consignment->vehicle_type ?? 'N/A' }}</div>
                            </div>
                            @if($vehicleId)
                    ...
                    <div class="trip-detail-card">
                        ...
                        <div class="trip-detail-row">
                            ...
                            <span class="trip-detail-value">{{ $consignment->consigner ?? 'N/A' }}</span>
                        </div>
                        <div class="trip-detail-row">
                            <span class="trip-detail-label">Weight</span>
                            <span class="trip-detail-value">{{ $consignment->weight ?? 'N/A' }} Tons</span>
                        </div>
                        <div class="trip-detail-row">
                            <span class="trip-detail-label">Packages</span>
                            <span class="trip-detail-value">{{ $consignment->packages ?? 'N/A' }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <form class="consignment-form" method="POST" action="{{ route('admin.customer-consignment.charges-advance.store', $consignment->id) }}">
        @csrf
        ...
        <div class="freight-options">
                ...
                <div class="freight-fields">
                    <div class="field-row">
                        <div class="form-group" style="flex: 1;">
                            ...
                            <input type="number" name="freight_weight" value="{{ old('freight_weight', isset($consignment) ? $consignment->freight_weight : '') }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;" oninput="calculateFreightCost()">
                        </div>
                        <div class="form-group" style="flex: 1;">
                            ...
                            <select name="weight_unit" style="width: 100%; height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C; appearance: none; background: white;">
                                <option {{ (!old('weight_unit', isset($consignment) ? $consignment->weight_unit : '')) ? 'selected' : '' }}>Unit..</option>
                                <option value="Kg" {{ (old('weight_unit', isset($consignment) ? $consignment->weight_unit : '') == 'Kg') ? 'selected' : '' }}>Kg</option>
                                <option value="Tons" {{ (old('weight_unit', isset($consignment) ? $consignment->weight_unit : '') == 'Tons') ? 'selected' : '' }}>Tons</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        <label style="font-size: 16px; font-weight: 500; color: #313131;">Rate / Unit</label>
                        <input type="number" name="rate_per_unit" value="{{ old('rate_per_unit', isset($consignment) ? $consignment->rate_per_unit : '') }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;" oninput="calculateFreightCost()">
                    </div>
                </div>
            </div>
            ...
            <div class="freight-option">
                ...
                <div class="freight-fields">
                    <div class="form-group">
                        ...
                        <input type="number" name="total_packages" value="{{ old('total_packages', isset($consignment) ? $consignment->total_packages : '') }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;" oninput="calculateFreightCost()">
                    </div>
                    <div class="form-group">
                        <label style="font-size: 16px; font-weight: 500; color: #313131;">Rate / Package</label>
                        <input type="number" name="rate_per_package" value="{{ old('rate_per_package', isset($consignment) ? $consignment->rate_per_package : '') }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;" oninput="calculateFreightCost()">
                    </div>
                </div>
            </div>
            ...
            <div class="freight-option">
                ...
                <div class="freight-fields">
                    <div class="form-group">
                        <label style="font-size: 16px; font-weight: 500; color: #313131;">Fixed Freight Cost</label>
                        <input type="number" name="fixed_cost" value="{{ old('fixed_cost', isset($consignment) ? $consignment->fixed_cost : '') }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;" oninput="calculateFreightCost()">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="calculated-cost">
            Calculated Freight Cost = <span id="calculated-freight">0.00</span>
        </div>
        ...
        <div class="itemized-section">
            ...
            <div class="expense-row">
                ...
                <div class="form-group">
                    <label style="font-size: 16px; font-weight: 500; color: #313131;">Amount</label>
                    <input type="number" name="expense_amount[]" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 17px; font-size: 14px; font-weight: 300; color: #000;" oninput="calculateTotalExpenses()">
                </div>
                <div class="form-group">
                ...
                <div class="summary-fields">
                    <div class="summary-column">
                        <div class="summary-field">
                            ...
                            <div class="value">{{ $consignment->consigner ?? 'N/A' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Route</label>
                            <div class="value" id="route-display">{{ $consignment->source_city ?? 'N/A' }} → {{ $consignment->dest_city ?? 'N/A' }}</div>
                        </div>
                    </div>
                    <div class="summary-column">
                        <div class="summary-field">
                            <label>Scheduled Pickup Date & Time</label>
                            <div class="value">{{ $consignment->pickup_datetime ? $consignment->pickup_datetime->format('d M Y, h:i A') : 'N/A' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Expected Delivery Date (Calculated)</label>
                            <div class="value">{{ $consignment->delivery_date ? $consignment->delivery_date->format('d M Y') : 'N/A' }}</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="summary-fields">
                    <div class="summary-column">
                        <div class="summary-field">
                            ...
                            <div class="value" id="freight-details-distance">{{ $consignment->total_distance ? number_format($consignment->total_distance, 2) . ' km' : 'Calculating...' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Total Travel Time</label>
                            <div class="value" id="freight-details-time">{{ $consignment->total_travel_time ?? 'Calculating...' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Load / Weight</label>
                            <div class="value">{{ $consignment->weight ?? 'N/A' }} Tons</div>
                        </div>
                    </div>
                    <div class="summary-column">
                        <div class="summary-field">
                            <label>Assigned Driver</label>
                            <div class="value">{{ $consignment->assigned_driver ?? 'N/A' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Vehicle Type</label>
                            <div class="value">{{ $consignment->vehicle_type ?? 'N/A' }}</div>
                        </div>
                        <div class="summary-field">
                            <label>Vehicle No</label>
                            <div class="value">{{ $consignment->assigned_vehicle_no ?? 'N/A' }}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="final-section">
            <div class="form-group final-input">
                ...
                <input type="text" name="final_notes" value="{{ old('final_notes', isset($consignment) ? $consignment->final_notes : '') }}" placeholder="Instructions / Remarks" style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 17px; font-size: 14px; font-weight: 400; color: #000;">
            </div>
        ...
        <div class="form-actions" style="margin-top: 30px; display: flex; justify-content: space-between; width: 920px; margin-left: auto; margin-right: auto;">
            <a href="{{ route('admin.customer-consignment.freight-assignment', $consignment->id) }}" class="btn btn-secondary">Back</a>
...
<script>
// Fetch route cities from Google Maps on page load
document.addEventListener('DOMContentLoaded', function() {
    fetchRouteCities();
});

async function fetchRouteCities() {
    const pickupLocation = '{{ $consignment->pickup_location ?? "" }}';
    const deliveryLocation = '{{ $consignment->delivery_location ?? "" }}';
    const sourceCity = '{{ $consignment->source_city ?? "" }}';
    const destCity = '{{ $consignment->dest_city ?? "" }}';
    
    // If both cities are already available, use them
    if (sourceCity && destCity) {
        updateRouteDisplay(sourceCity, destCity);
        return;
    }
    
    // Fetch pickup city
    let pickupCity = sourceCity;
    if (!pickupCity && pickupLocation) {
        pickupCity = await extractCityFromLocation(pickupLocation);
    }
    
    // Fetch delivery city
    let deliveryCity = destCity;
    if (!deliveryCity && deliveryLocation) {
        deliveryCity = await extractCityFromLocation(deliveryLocation);
    }
    
    updateRouteDisplay(pickupCity || 'N/A', deliveryCity || 'N/A');
}

async function extractCityFromLocation(address) {
    ...
    if (apiKey && apiKey.length > 10) {
        try {
            const city = await geocodeToCity(address, apiKey);
    ...
    // Fallback: Parse address string to extract city
    return parseCityFromAddress(address);
}

async function geocodeToCity(address, apiKey) {
    try {
...
function parseCityFromAddress(address) {
    ...
    // Known UAE cities and their variations
    const knownCities = [
        { name: 'Dubai', variations: ['dubai'] },
        { name: 'Abu Dhabi', variations: ['abu dhabi', 'abu-dhabi', 'abudhabi'] },
        { name: 'Sharjah', variations: ['sharjah'] },
        { name: 'Ajman', variations: ['ajman'] },
        { name: 'Fujairah', variations: ['fujairah', 'fujera'] },
        { name: 'Ras Al Khaimah', variations: ['ras al khaimah', 'ras al kaimah', 'rak', 'rasalkhaimah'] },
        { name: 'Umm Al Quwain', variations: ['umm al quwain', 'umm al qawein', 'uaq'] },
        { name: 'Al Ain', variations: ['al ain', 'alain'] }
    ];
    ...
    for (let city of knownCities) {
        for (let variation of city.variations) {
    ...
    // Parse address by comma-separated parts
    const parts = address.split(',').map(p => p.trim());
    
    // Common patterns:
    // "Al Qusais, Dubai, UAE" -> Dubai
    // "Deira, Dubai" -> Dubai
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
        ...
        if (lowerPart.includes('uae') || 
            lowerPart.includes('united arab') || 
            lowerPart.includes('emirates')) {
            continue;
        }
        
        // Skip common non-city terms
        if (lowerPart.includes('road') || 
            lowerPart.includes('street') || 
            lowerPart.includes('ave') || 
            lowerPart.includes('boulevard') ||
            lowerPart.includes('highway') ||
            lowerPart.includes('district') ||
            lowerPart.includes('area')) {
            continue;
        }
        
        // Check if this part looks like a city (reasonable length, not too short)
    ...
    // Return second-to-last part as it might be the city (after area, before country)
    if (parts.length >= 2) {
...
function updateRouteDisplay(pickupCity, deliveryCity) {
    const routeDisplay = document.getElementById('route-display');
    if (routeDisplay) {
        routeDisplay.textContent = `${pickupCity} → ${deliveryCity}`;
    }
}

// Calculate Freight Cost based on selected option
function calculateFreightCost() {
    ...
    // Priority: Fixed Rate > Rate By Package > Rate By Weight
    if (fixedCost > 0) {
        calculatedCost = fixedCost;
    } else if (totalPackages > 0 && ratePerPackage > 0) {
        calculatedCost = totalPackages * ratePerPackage;
    } else if (freightWeight > 0 && ratePerUnit > 0) {
        calculatedCost = freightWeight * ratePerUnit;
    }
    
    document.getElementById('calculated-freight').textContent = calculatedCost.toFixed(2);
}

// Calculate Total Expenses from itemized expenses
...
function addExpenseRow() {
    ...
    // Add oninput event to new expense amount field
    const newExpenseAmount = newRow.querySelector('input[name="expense_amount[]"]');
    if (newExpenseAmount) {
        newExpenseAmount.setAttribute('oninput', 'calculateTotalExpenses()');
    }
...
function showStaticTripMap() {
    ...
    var pickupAddr = "{{ isset($consignment) ? ($consignment->pickup_location ?? $consignment->source_city ?? 'Dubai, UAE') : 'Dubai, UAE' }}";
    var deliveryAddr = "{{ isset($consignment) ? ($consignment->delivery_location ?? $consignment->dest_state ?? $consignment->dest_city ?? 'Abu Dhabi, UAE') : 'Abu Dhabi, UAE' }}";
...
// Calculate distance and time on page load
document.addEventListener('DOMContentLoaded', function() {
    calculateFreightDistance();
});

function calculateFreightDistance() {
    // Check if Google Maps is loaded
    if (typeof google === 'undefined' || typeof google.maps === 'undefined') {
        // Try loading Google Maps dynamically
        var apiKey = "{{ env('GOOGLE_MAPS_API_KEY') }}";
        if (apiKey) {
            ...
            document.getElementById('freight-details-distance').textContent = 'N/A';
            document.getElementById('freight-details-time').textContent = 'N/A';
        }
        return;
    }
    
    const pickupAddr = "{{ isset($consignment) ? ($consignment->pickup_location ?? $consignment->source_city ?? '') : '' }}";
    const deliveryAddr = "{{ isset($consignment) ? ($consignment->delivery_location ?? $consignment->dest_city ?? '') : '' }}";
    ...
    directionsService.route(request, function(response, status) {
        if (status === 'OK') {
            ...
            // Update distance in Freight Details
            const distanceText = leg.distance.text;
            const distanceValue = parseFloat(distanceText.replace(/[^0-9.]/g, ''));
            if (!isNaN(distanceValue)) {
                document.getElementById('freight-details-distance').textContent = distanceValue.toFixed(2) + ' km';
            } else {
                document.getElementById('freight-details-distance').textContent = distanceText;
            }
            
            // Update travel time in Freight Details
            document.getElementById('freight-details-time').textContent = leg.duration.text;
        } else {
            console.error('Directions request failed: ' + status);
            // Keep the saved value if available
            if (document.getElementById('freight-details-distance').textContent === 'Calculating...') {
                document.getElementById('freight-details-distance').textContent = 'N/A';
            }
        }
    });
