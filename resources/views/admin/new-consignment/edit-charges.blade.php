...
<style>
    ...
    .trip-route-line {
        ...
        width: 80%;
        height: 4px;
        background: linear-gradient(90deg, #317ff1 0%, #33C17F 100%);
        z-index: 1;
    }
...
<div class="dashboard-wrapper">
    ...
    <div class="consignment-container">
        ...
        @php
        $expenseTypes = is_array($transport->expense_types) ? $transport->expense_types : [];
        $expenseAmounts = is_array($transport->expense_amounts) ? $transport->expense_amounts : [];
        $expenseRemarks = is_array($transport->expense_remarks) ? $transport->expense_remarks : [];
        @endphp

        <form class="consignment-form" method="POST" action="{{ route('admin.charges-advance.update', $transport->id) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="freight-fields">
                        <div class="field-row">
                            <div class="form-group" style="flex: 1;">
                                ...
                                <input type="text" name="freight_weight" value="{{ old('freight_weight', $transport->freight_weight) }}" placeholder="Number.." style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C;">
                            </div>
                            <div class="form-group" style="flex: 1;">
                                ...
                                <select name="weight_unit" style="width: 100%; height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 12px; font-size: 14px; font-weight: 300; color: #4C4C4C; appearance: none; background: white;">
                                    <option {{ (!old('weight_unit', $transport->weight_unit)) ? 'selected' : '' }}>Unit..</option>
                                    <option value="Kg" {{ (old('weight_unit', $transport->weight_unit) == 'Kg') ? 'selected' : '' }}>Kg</option>
                                    <option value="Tons" {{ (old('weight_unit', $transport->weight_unit) == 'Tons') ? 'selected' : '' }}>Tons</option>
                                </select>
                            </div>
                        </div>
        ...
        <div class="calculated-cost" style="margin-top: 20px; font-size: 30px;">
                    ...
                    <div class="summary-fields">
                        <div class="summary-column">
                            ...
                            <div class="summary-field">
                                ...
                                <div class="value">
                                    @php
                                    $sourceCity = $transport->source_city ?? $transport->pickup_location ?? $transport->source_state ?? 'N/A';
                                    $destArea = $transport->dest_state ?? $transport->delivery_location ?? $transport->dest_city ?? 'N/A';
                                    @endphp
                                    {{ $sourceCity }} → {{ $destArea }}
                                </div>
                            </div>
                    ...
                    <div class="summary-fields">
                        <div class="summary-column">
                            <div class="summary-field">
                                ...
                                <div class="value" id="total-distance-value">
                                    @if($transport->total_distance)
                                        {{ number_format($transport->total_distance, 2) }} km
                                    @else
                                        Calculating...
                                    @endif
                                </div>
                            ...
                            <div class="summary-field">
                                ...
                                <div class="value" id="travel-time-value">
                                    @if($transport->total_travel_time)
                                        {{ $transport->total_travel_time }}
                                    @else
                                        Calculating...
                                    @endif
                                </div>
            ...
            <div class="final-section">
                <div class="form-group final-input">
                    ...
                    <input type="text" name="final_notes" value="{{ old('final_notes', $transport->final_notes) }}" placeholder="Instructions / Remarks" style="height: 45px; border: 1px solid #313131; border-radius: 10px; padding: 0 17px; font-size: 14px; font-weight: 400; color: #000;">
                </div>
                <label class="confirm-btn">
                    <input type="checkbox" name="confirm_booking" {{ old('confirm_booking') ? 'checked' : '' }} required>
            ...
            <div class="form-actions" style="margin-top: 30px; display: flex; justify-content: space-between; width: 920px; margin-left: auto; margin-right: auto;">
                <div style="display: flex; gap: 10px;">
                    ...
                    <a href="{{ route('admin.freight-assignment.edit', $transport->id) }}" class="btn btn-secondary">Back</a>
                </div>
...
<script>
    ...
    var pickupLocation = "{{ $transport->pickup_location ?? $transport->source_city ?? '' }}";
    var deliveryLocation = "{{ $transport->delivery_location ?? $transport->dest_city ?? $transport->dest_state ?? '' }}";
    ...
    if (!deliveryLocation || deliveryLocation === 'N/A') deliveryLocation = "Abu Dhabi, UAE";
    
    function updateDistanceDisplay(distance, travelTime) {
        var distanceElement = document.getElementById('total-distance-value');
        var travelTimeElement = document.getElementById('travel-time-value');
        
        if (distanceElement) {
            if (distance && distance !== 'N/A') {
                distanceElement.textContent = parseFloat(distance).toFixed(2) + ' km';
            } else {
                distanceElement.textContent = 'N/A';
            }
        }
        if (travelTimeElement) {
            travelTimeElement.textContent = travelTime || 'N/A';
        }
    }
    
    function calculateRouteDistance() {
        // If we have stored values, use them
        if (storedDistance && storedTravelTime) {
            updateDistanceDisplay(storedDistance, storedTravelTime);
            return;
        }
        ...
        if (typeof google === 'undefined' || typeof google.maps === 'undefined') {
            ...
            if (googleMapsApiKey) {
                ...
                document.getElementById('total-distance-value').textContent = 'N/A';
                document.getElementById('travel-time-value').textContent = 'N/A';
            }
            return;
        }
        ...
        try {
            ...
            directionsService.route(request, function(response, status) {
                if (status === 'OK') {
                    ...
                    var distanceValue = parseFloat(distanceText.replace(/[^0-9.]/g, ''));
    ...
    function addExpenseRow() {
        var expenseContainer = document.querySelector('.expense-row').parentElement;
        var newRow = document.querySelector('.expense-row').cloneNode(true);
        newRow.querySelectorAll('input').forEach(function(input) { input.value = ''; });
        newRow.querySelector('.add-btn').setAttribute('onclick', 'removeExpenseRow(this)');
        newRow.querySelector('.add-btn').textContent = '-';
    ...
    function validateForm() {
        ...
        var freightWeight = document.querySelector('input[name="freight_weight"]').value.trim();
        var ratePerUnit = document.querySelector('input[name="rate_per_unit"]').value.trim();
        var totalPackages = document.querySelector('input[name="total_packages"]').value.trim();
        var ratePerPackage = document.querySelector('input[name="rate_per_package"]').value.trim();
        var fixedCost = document.querySelector('input[name="fixed_cost"]').value.trim();

        var hasRateByWeight = freightWeight && ratePerUnit;
        var hasRateByPackage = totalPackages && ratePerPackage;
        var hasFixedRate = fixedCost;
        ...
        var expenseAmounts = document.querySelectorAll('input[name="expense_amount[]"]');
        expenseAmounts.forEach(function(input, index) {
        ...
        var confirmBooking = document.querySelector('input[name="confirm_booking"]');
    ...
    function showErrorAlert(errors) {
        ...
        alertDiv.innerHTML = '<ul>' + errors.map(function(e) { return '<li>' + e + '</li>'; }).join('') + '</ul>';

        var form = document.querySelector('.consignment-form');
    ...
    document.addEventListener('input', function(e) {
        if (e.target.classList.contains('form-control') || e.target.tagName === 'INPUT') {
            ...
            var errorDiv = e.target.parentElement.querySelector('.field-error');
    ...
    function showStaticTripMap() {
        ...
        if (mapContainer) {
            mapContainer.innerHTML = '<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: #f0f0f0; border-radius: 15px;">' +
                ...
                '<div style="font-size: 16px; color: #666;">Route: ' + pickupLocation + ' → ' + deliveryLocation + '</div>' +
                '<div style="font-size: 14px; color: #999; margin-top: 10px;">Google Maps API Required</div>' +
                '</div>';
        }
        // Update stats with stored values
        if (storedDistance) {
            document.getElementById('trip-stat-distance').textContent = storedDistance;
        }
        if (storedTravelTime) {
            document.getElementById('trip-stat-time').textContent = storedTravelTime;
        }
    }
    ...
    function calculateTripRoute() {
        ...
        var pickupAddr = "{{ $transport->pickup_location ?? $transport->source_city ?? 'Dubai, UAE' }}";
        var deliveryAddr = "{{ $transport->delivery_location ?? $transport->dest_state ?? $transport->dest_city ?? 'Abu Dhabi, UAE' }}";
