...
<div class="form-container-wrapper">
    ...
    <div class="form-container">
        ...
        <!-- Team Member Form -->
        <form class="member-form" method="POST" action="{{ route('admin.team-members.update', $teamMember->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="name" placeholder="e.g., John Doe" value="{{ old('name', $teamMember->name) }}" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" placeholder="e.g., john.doe@example.com" value="{{ old('email', $teamMember->email) }}" required>
                    @error('email')
                        <span style="color: #ED5A68; font-size: 12px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    <label><i class="fas fa-phone" style="margin-right: 8px;"></i>Mobile</label>
                    <input type="tel" name="mobile" placeholder="e.g., +1 234 567 8900" value="{{ old('mobile', $teamMember->mobile) }}">
                    @error('mobile')
                        <span style="color: #ED5A68; font-size: 12px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="role_id" required>
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" {{ old('role_id', $teamMember->role_id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
                    @error('role_id')
                        <span style="color: #ED5A68; font-size: 12px;">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="department" placeholder="e.g., Operations" value="{{ old('department', $teamMember->department) }}">
                    @error('department')
                        <span style="color: #ED5A68; font-size: 12px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="position" placeholder="e.g., Senior Manager" value="{{ old('position', $teamMember->position) }}">
                    @error('position')
                        <span style="color: #ED5A68; font-size: 12px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="date_of_joining" value="{{ old('date_of_joining', $teamMember->date_of_joining ? $teamMember->date_of_joining->format('Y-m-d') : '') }}">
                    ...
                    <div class="file-upload-group">
                        ...
                        @if($teamMember->profile_image)
                            <div style="margin-bottom: 10px;">
                                <img src="{{ asset($teamMember->profile_image) }}" alt="Current Profile" style="width: 80px; height: 80px; border-radius: 10px; object-fit: cover; border: 1px solid #ddd;">
                            </div>
                        @endif
