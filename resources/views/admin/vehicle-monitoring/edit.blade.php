...
<div class="form-container-wrapper">
    ...
    <div class="form-container">
        ...
        <!-- Vehicle Form -->
        <form class="vehicle-form" method="POST" action="{{ route('admin.vehicle-monitoring.update', $vehicle->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select name="brand" required>
                        ...
                        @foreach($brands as $brand)
                            <option value="{{ $brand->name }}" {{ $vehicle->brand == $brand->name ? 'selected' : '' }}>{{ $brand->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="model" placeholder="e.g., Activa 6G" value="{{ $vehicle->model }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-id-card" style="margin-right: 8px;"></i>Vehicle Number<span class="required">*</span></label>
                    <input type="text" name="vehicle_number" placeholder="e.g., MH01AB1234" value="{{ $vehicle->vehicle_number }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-calendar-alt" style="margin-right: 8px;"></i>Purchase Date<span class="required">*</span></label>
                    <input type="date" name="purchase_date" value="{{ $vehicle->purchase_date->format('Y-m-d') }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-calendar" style="margin-right: 8px;"></i>Registration Year<span class="required">*</span></label>
                    <input type="number" name="registration_year" placeholder="e.g., 2023" value="{{ $vehicle->registration_year }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-palette" style="margin-right: 8px;"></i>Color<span class="required">*</span></label>
                    <input type="text" name="color" placeholder="e.g., White" value="{{ $vehicle->color }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="average" placeholder="e.g., 12 km/l" value="{{ $vehicle->average }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-weight-hanging" style="margin-right: 8px;"></i>Max Weight<span class="required">*</span></label>
                    <input type="text" name="max_weight" placeholder="e.g., 1000 kg" value="{{ $vehicle->max_weight }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-road" style="margin-right: 8px;"></i>Current Odometer<span class="required">*</span></label>
                    <input type="text" name="current_odometer" placeholder="e.g., 50000 km" value="{{ $vehicle->current_odometer }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-shield-alt" style="margin-right: 8px;"></i>Insurance Valid Till<span class="required">*</span></label>
                    <input type="text" name="insurance_valid_till" placeholder="e.g., 31/12/2025" value="{{ $vehicle->insurance_valid_till }}" required>
                </div>

                <div class="form-group">
                    <label><i class="fas fa-file-alt" style="margin-right: 8px;"></i>PUC Expiry<span class="required">*</span></label>
                    <input type="text" name="puc_expiry" placeholder="e.g., 31/12/2025" value="{{ $vehicle->puc_expiry }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="driver-dropdown" id="driver-dropdown">
                        ...
                        <div class="driver-options" id="driver-options">
                            ...
                            <div class="driver-option {{ $vehicle->driver_id == $driver->id ? 'selected' : '' }}" data-value="{{ $driver->id }}">
                                <div class="driver-avatar">
                        ...
                        <select name="driver_id" class="hidden-select" id="hidden-driver-select">
                            <option value=""></option>
                            @foreach($drivers as $driver)
                                <option value="{{ $driver->id }}" {{ $vehicle->driver_id == $driver->id ? 'selected' : '' }}></option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="attach-section">
                ...
                <div class="file-upload-grid">
                    <div class="file-upload-group">
                        ...
                        <!-- Current Vehicle Photo Preview -->
                        <div class="image-preview-container">
                            <div class="image-preview" id="vehicle-photo-preview">
                                @if($vehicle->image_path)
                                    <img src="{{ asset($vehicle->image_path) }}" alt="Current Vehicle Photo">
                                @else
                                    <div class="placeholder">No Photo</div>
                                @endif
                            </div>
                            <small style="color: #666; font-size: 12px;">Current Vehicle Photo</small>
                        </div>
                        ...
                        <small style="color: #666; font-size: 12px;">Accepted formats: JPEG, PNG, JPG, GIF. Maximum size: 5MB. Leave empty to keep current photo</small>
                    </div>

                    <div class="file-upload-group">
                        <label>Vehicle Documents</label>

                        <!-- Current Vehicle Documents Preview -->
                        <div class="image-preview-container">
                            <div class="image-preview" id="vehicle-documents-preview">
                                @if($vehicle->documents_path)
                                    @if(pathinfo($vehicle->documents_path, PATHINFO_EXTENSION) === 'pdf')
                                        <div class="placeholder">PDF Document</div>
                                    @else
                                        <img src="{{ asset($vehicle->documents_path) }}" alt="Current Document">
                                    @endif
                                @else
                                    <div class="placeholder">No Document</div>
                                @endif
                            </div>
                            <small style="color: #666; font-size: 12px;">Current Vehicle Documents</small>
                        </div>
                        ...
                        <small style="color: #666; font-size: 12px;">Accepted formats: PDF, DOC, DOCX, JPG, PNG, JPEG. Maximum size: 5MB. Leave empty to keep current documents</small>
                    </div>
                </div>
            </div>
