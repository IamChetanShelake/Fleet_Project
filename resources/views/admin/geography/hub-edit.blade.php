...
<div class="dashboard-wrapper">
    ...
    <div class="geography-container">
        <div class="geography-header">
            <h1 class="geography-title">Edit Hub</h1>
            <div class="header-actions">
                <a href="{{ route('admin.hubs.show', $hub->id) }}" class="btn-view">
                    <i class="fas fa-eye"></i> View Hub
                </a>
        ...
        <form class="geography-form" method="POST" action="{{ route('admin.hubs.update', $hub->id) }}">
            @csrf
            @method('PUT')

            <div class="section-header">
                <span class="section-icon">🏢</span>
                <h2>Edit Hub Details</h2>
            </div>
            ...
            <div class="form-grid">
                <div class="form-section">
                        ...
                        <div class="select-wrapper">
                            <select name="country_id" required>
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ old('country_id', $hub->country_id) == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="select-wrapper">
                            <select name="city_id" required>
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" {{ old('city_id', $hub->city_id) == $city->id ? 'selected' : '' }}>
                                        {{ $city->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="code" value="{{ old('code', $hub->code) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="select-wrapper">
                            <select name="status">
                                <option value="1" {{ old('status', $hub->status) ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ !old('status', $hub->status) ? 'selected' : '' }}>Inactive</option>
                            </select>
                            <svg width="10" height="7" viewBox="0 0 10 7" fill="none"><path d="M1 1l4 4 4-4" stroke="#6C6C6C" stroke-width="1.5"/></svg>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    <div class="form-group">
                        <label>Address</label>
                        <textarea name="address" rows="3" placeholder="Full address of the hub">{{ old('address', $hub->address) }}</textarea>
                    </div>

                    <div class="form-group">
                        <label>Contact Person</label>
                        <input type="text" name="contact_person" value="{{ old('contact_person', $hub->contact_person) }}">
                    </div>

                    <div class="form-group">
                        <label>Contact Number</label>
                        <input type="text" name="contact_number" value="{{ old('contact_number', $hub->contact_number) }}">
                    </div>
                </div>
            </div>

            <div class="form-actions">
                <a href="{{ route('admin.hubs.show', $hub->id) }}" class="btn btn-secondary">Cancel</a>
